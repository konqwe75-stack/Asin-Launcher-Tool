...
function openToolTabs() {
  ...
  const asins = [...new Set(document.getElementById('asinInput').value.trim().split(/\s+/))];
  ...
  const urls = {
    ...
    imagedebugger: asin => `https://variations.amazon.com/ImageDebugger/#asin=${asin}&region=${region.toLowerCase()}`,
    bqe: asin => `https://browse-query-editor-${region.toLowerCase()}.aka.amazon.com/?marketplaceId=${marketplaceId}&userQuery=${asin}`,
    ...
    asintool: asin => `https://csi.amazon.com/view?view=asin_creator&item_id=${asin}&marketplace_id=${marketplaceId}&region=${region}`,
    marketplacedata: asin => `https://csi.amazon.com/view?view=domains_for_asin&item_id=${asin}&marketplace_id=${marketplaceId}&region=${region}`
  };
...
window.onload = () => {
  ...
  document.querySelectorAll("li").forEach(li => li.addEventListener('dragstart', drag));
};